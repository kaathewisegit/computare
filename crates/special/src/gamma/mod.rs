...
use computare_core::{Float, FloatMath, debug_panic};

pub(crate) const MAX_GAMMA: f64 = 171.6243769563027;
...
fn ln_gamma_large_x(x: f64) -> f64 {
    let q = (x - 0.5) * x.ln() - x + f64::LN_SQRT_2PI;
    if x > 1.0e8 {
        return q;
    }
    let p = 1.0 / (x * x);
...
pub(crate) fn ln_gamma_sgn(x: f64, sign: &mut i32) -> f64 {
    *sign = 1;
