...
pub trait MatrixMut<T>: Matrix<T> {
    unsafe fn u_at_mut(&mut self, row: usize, col: usize) -> &mut T;

    fn at_mut(&mut self, row: usize, col: usize) -> &mut T {
        assert!(row < self.num_rows() && col < self.num_rows());
        unsafe { self.u_at_mut(row, col) }
    }
}
...
pub unsafe fn u_swap_rows<T, M>(m: &mut M, a: usize, b: usize)
where
    M: MatrixMut<T> + ?Sized,
{
    if a == b {
    ...
    let a_ptr = unsafe { m.u_at_mut(a, 0) } as *mut T;
    let b_ptr = unsafe { m.u_at_mut(b, 0) } as *mut T;
